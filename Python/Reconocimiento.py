...
class Reconocimiento:
    ...
    def PorcionReconocimiento (cnts, gray,min_w=80, max_w=110, min_h=25, max_h=52, ratio=2.5):
        ...
        for c in cnts:
            ...
            if len(approx) == 4 and 1000 < area < 10000 and (max_w > w > min_w) and (max_h > h > min_h):
                if aspect_ratio > ratio:
                    ...
                    cv2.waitKey(0)
                    return
    def lecDigitos(BinaryPlaca, textAux = "", psm=7):
        # EXTRAE EL TEXTO Y LO MUESTRA POR CONSOLA
        # NORMALIZACION DE CARACTERES
        alphanumeric = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
        options = "-c tessedit_char_whitelist={}".format(alphanumeric)
        options += " --psm {}".format(psm)
        text = textAux + pytesseract.image_to_string(BinaryPlaca, config=options)
